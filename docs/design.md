...
*   **2.1. Manifest (`manifest.json`)**
    ...
    *   **Key Declarations:**
        ...
        *   `host_permissions`: `*://*.linkedin.com/*` (for content script injection), `https://generativelanguage.googleapis.com/` (or the specific Gemini API endpoint, for background script API calls).
...
*   **2.3. Background Service Worker (`background.js`)**
    ...
    *   **Responsibilities:**
        *   **API Key Management:** Retrieve the stored Gemini API key from `chrome.storage`.
        ...
        *   **API Interaction (F3, F5):** Construct and send requests to the Google Gemini API using `fetch`, including the prompt, post content, JSON schema instruction, language instruction, and the user's API key. Handle API responses (success, errors, rate limits). *API key is handled securely here.*
        *   **Data Processing:** Parse the JSON response from Gemini.
...
*   **2.4. Options Page (`options.html`, `options.js`, `options.css`)**
    ...
    *   **Responsibilities:**
        *   **UI:** Provide an input field for the Gemini API Key (F7) and a save button. Styled with Bootstrap.
        *   **API Key Storage:** On save, store the entered API key securely using `chrome.storage.sync.set()`. Provide user feedback on save (success/failure).
        *   **API Key Retrieval:** On load, attempt to retrieve and display the currently saved API key (consider masking).
...
*   **Initial Generation (Simplified Push):**
    ...
    4.  **Background SW:** Gets API key. Calls Gemini. Gets response. Sends result (`GENERATION_SUCCESS` or `GENERATION_ERROR`) back to Content Script.
